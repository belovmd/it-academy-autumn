...
2. Определите, можно ли отломить от шоколадки ровно k долек за некоторое
 количество разломов.
3. Определите, можно ли отломить от шоколадки ровно k долек с помощью
 t разломов
...
def chocolate(n, m, k, t):
    ...
    # если последний кусок, то кол-во разломов совпадает с кол-вом долек,
    # кроме последей дольки
    if k > area - maxlen:
        if k == area:
            ans3 = k - 1 <= t
        else:
